...
static void vnn_GetImgHandleBuffer
    (
    uint32_t width,
    uint32_t height,
    uint32_t channels,
...
static void vnn_ReadJpegImage_Align
    (
    const char *name,
    uint32_t width,
    uint32_t height,
    ...
    )
{
    ...
    if (buffer_img == NULL)
    {
        vnn_GetImgHandleBuffer(width, height, channels,
            IMAGE_ADDR_ALIGN_START_SIZE, IMAGE_ADDR_ALIGN_BLOCK_SIZE);
    }

    vnn_ConvertJpegToBmpData(bmpFile, buffer_img_align_addr, width, height, channels);
    ...
    )
{
    ...
    for(i = 0; i < sz; i++)
    {
		...
		tensorData[stride * i] = fval;
//        status = vsi_nn_Float32ToDtype(fval, &tensorData[stride * i], &tensor->attr.dtype);
//        _CHECK_STATUS(status, error);
    }
    if(tensorFile)fclose(tensorFile);
    tensorFile=fopen("input.bin","wb");
    fwrite(tensorData,stride * sz * sizeof(uint8_t),1,tensorFile);
    fclose(tensorFile);
#else
    tensorFile = fopen("input.bin", "rb");
    fread(tensorData,stride * sz * sizeof(uint8_t),1,tensorFile); 
    if(tensorFile)fclose(tensorFile);
#endif
    return tensorData;
error:
    ...
    )
{
    ...
    if(fileType == NN_FILE_JPG)
    {
        ...
        vnn_ReadJpegImage_Align(image_name, attr.size[0], attr.size[1], attr.size[2]);
        status = vsi_nn_op_imageprocess_single_node(graph, &attr, &imageprocess,
            buffer_img_align_addr, tensor);
    }
    else
    {
        printf("vnn_PreProcessDncnn_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        _CHECK_STATUS(status, final);
    }
