...
det_status_t model_create(const char * data_file_path, dev_type type)
{
	...
	switch (type) {
		case DEV_REVA:
			sprintf(model_path, "%s%s", data_file_path, "/yolov4_7d.nb");
			break;
		case DEV_REVB:
			sprintf(model_path, "%s%s", data_file_path, "/yolov4_88.nb");
			break;
		case DEV_MS1:
			sprintf(model_path, "%s%s", data_file_path, "/yolov4_99.nb");
			break;
		default:
			break;
	}
	g_graph = vnn_CreateYolov4(model_path, NULL,
...
{
	yolov4_preprocess(imageData, data);
}
...
det_status_t model_getresult(pDetResult resultData, uint8_t* data)
{
	...
	yolov4_postprocess(g_graph, resultData);
...
{
	vnn_ReleaseYolov4(g_graph, TRUE);
	g_graph = NULL;
}
