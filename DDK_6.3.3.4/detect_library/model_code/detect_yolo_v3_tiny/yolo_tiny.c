...
det_status_t model_create(const char * data_file_path, dev_type type)
{
	...
	switch (type) {
		case DEV_REVA:
			sprintf(model_path, "%s%s", data_file_path, "/yolotiny_7d.nb");
			break;
		case DEV_REVB:
			sprintf(model_path, "%s%s", data_file_path, "/yolotiny_88.nb");
			break;
		case DEV_MS1:
			sprintf(model_path, "%s%s", data_file_path, "/yolotiny_99.nb");
			break;
		default:
			break;
	}
	g_graph = vnn_CreateYolotiny(model_path, NULL,
...
{
	yolo_tiny_preprocess(imageData, data);
}
...
det_status_t model_getresult(pDetResult resultData, uint8_t* data)
{
	...
	yolo_tiny_postprocess(g_graph, resultData);
...
{
	vnn_ReleaseYolotiny(g_graph, TRUE);
	g_graph = NULL;
}
