...
det_status_t model_create(const char * data_file_path, dev_type type)
{
	...
	switch (type) {
		case DEV_REVA:
			...
			break;
		case DEV_REVB:
			sprintf(model_path, "%s%s", data_file_path, "/densenet_ctc.nb");
			break;
		case DEV_MS1:
...
det_status_t densenet_ctc_model_getresult(char* result, int* result_len, uint8_t* data)
{
	vsi_status status = VSI_FAILURE;
	det_status_t ret = DET_STATUS_ERROR;
	vsi_nn_tensor_t *tensor = vsi_nn_GetTensor(g_graph, g_graph->input.tensors[0]);
	...
	yolov3_postprocess(g_graph, result, result_len);
