...
export OPENCV_ROOT=$SDK_DIR/opencv4
...
export TOOLCHAIN=$AQROOT/../../toolchains/aarch64-none-linux-gnu/bin
export LIB_DIR=$AQROOT/../../toolchains/aarch64-none-linux-gnu/aarch64-none-linux-gnu/libc/lib
########################################################
...
make -f makefile_cv4.linux $BUILD_OPTIONS V_TARGET=clean
make -f makefile_cv4.linux $BUILD_OPTIONS V_TARGET=install  2>&1 | tee $AQROOT/linux_build_sample.log

########################################################
# other build/clean commands to build/clean specified items, eg.
#
# cd $AQROOT; make -f makefile_cv4.linux $BUILD_OPTIONS gal_core V_TARGET=clean || exit 1
# cd $AQROOT; make -f makefile_cv4.linux $BUILD_OPTIONS gal_core V_TARGET=install || exit 1
